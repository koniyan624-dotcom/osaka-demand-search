...
GitHub Actionsが10分おきに各鉄道会社の運行情報を取得し、自動でステータスを更新します。
...
| 近畿日本鉄道 | 近鉄大阪線・近鉄奈良線 |
| 南海電気鉄道 | 南海本線・南海高野線 |
| Osaka Metro | 大阪メトロ御堂筋線 |

手動のまま残っているのは **大阪モノレール・新幹線(山陽・東海道)** の2路線のみです（信頼できる無料の取得元が見つからず未対応）。

仕組みは [scripts/update-train-status.js](scripts/update-train-status.js) と [.github/workflows/update-train-status.yml](.github/workflows/update-train-status.yml) です。GitHubにpushするだけで動作し、追加のシークレット設定は不要です（Firestoreルールが「誰でも書き込み可」のため）。1社の取得に失敗しても他社の処理は止まらない設計にしています。
